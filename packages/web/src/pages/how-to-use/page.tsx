...
import {
  ...
  IndentLi,
  QuickStartItem,
} from "./HowToUseItem2.tsx";

/**
 * 文字をtertiaryカラーで表示するコンポーネント
 * @param param0 表示したい文字列を含むオブジェクト
 * @param param0.children 表示したい文字列
 * @returns tertiaryカラーで表示された文字列
 */
...
const HowToUsePage: React.FC = () => {
  return (
    <>
      {/* 使い方を項目に分け、それぞれをHowToUseItemコンポーネントで表示する */}
      ...
      <HowToUseItem title="Quick Start">
        ...
        <ol className="list-decimal list-inside mt-8">
          ...
          <QuickStartItem title="時間割の閲覧">
            画面上部のタブ
            (ヘッダー)の”View”を選択してください。先ほど選択した講義が追加されていることを確認できます。講義をクリックすると、その詳細情報が表示されます。
            <br />
      ...
      <HowToUseItem title="所属の設定">
        ...
        <Blue>必修科目や推奨科目が自動で登録</Blue>
        され、手間を省くことができます。仮に一つも授業が登録されていない、真っ新な状態から作成したい場合は
        <Blue>”必修を自動入力”</Blue>のチェックを外してください。
        {/* 必修の自動入力をした場合、同じ授業が複数登録される場合があるため、対処法を説明 */}
      ...
      <HowToUseItem title="時間割を編集 (Editモード)">
        ...
        <ol className="list-decimal list-inside mt-8">
          <IndentLi>
            画面上部のタブ(ヘッダー)から<Blue>”Edit”</Blue>
            を選択し、時間割編集モードに入ります。
          </IndentLi>
          <IndentLi>
      ...
      <HowToUseItem title="時間割を閲覧 (Viewモード)">
        画面上部のタブ(ヘッダー)から<Blue>”View”</Blue>
        を選択し、時間割編集モードに入ります。講義を選択すると、その詳細情報を見ることができます。その講義が
        <Blue>必修</Blue>もしくは<Blue>推奨</Blue>
